...
public class StreamAPI {
    public static void main(String[] args) {
        ...
        System.out.println(
                "Male count: " + maleCount + " " + "Female count: " + femaleCount + " " + "Other count: " + otherCount);
        ...
        System.out.println("Average male salary: " + avgMaleSalary + " Average female salary: " + avgFemaleSalary
        ...
        Employee minSalaryEmployee = employees.stream().min((e, f) -> Double.compare(e.getSalary(), f.getSalary()))
                .get();
        ...
        Map<String, Employee> employeeMaxSalaryInDepartment = employees.stream()
                .collect(Collectors.groupingBy(Employee::getDepartment,
                        Collectors.collectingAndThen(
                                Collectors.maxBy((e, f) -> Double.compare(e.getSalary(), f.getSalary())),
                                e -> e.get())));

        employeeMaxSalaryInDepartment
                .forEach((k, v) -> System.out.println(k + " --> " + v.getName() + " --> " + v.getSalary()));
        ...
        secondHighestSalariedEmployee.ifPresent(
                e -> System.out.println("Second highest salaried employee: " + e.getName() + " -> " + e.getSalary()));
        ...
        Map<String, Employee> employeeMinSalaryInDepartment = employees.stream()
                .collect(Collectors.groupingBy(Employee::getDepartment,
                        Collectors.collectingAndThen(
                                Collectors.minBy((e, f) -> Double.compare(e.getSalary(), f.getSalary())),
                                e -> e.get())));
        ...
        List<Employee> employeesWithSalaryGreaterThan5000 = employees.stream().filter(e -> e.getSalary() > 50000)
                .collect(Collectors.toList());
        ...
        List<Employee> employeesWhoKnowJavaAndCInDX = employees.stream()
                .filter(x -> x.getDepartment().equalsIgnoreCase("DX") && x.getTechStack().contains("Java")
        ...
        List<Employee> employeesSortedBasedOnTheirSalariesAndAge = employees.stream()
                .sorted((e, f) -> e.getSalary() == f.getSalary() ? Integer.compare(e.getAge(), f.getAge())
                        : Double.compare(e.getSalary(), f.getSalary()))
                .collect(Collectors.toList());

        employeesSortedBasedOnTheirSalariesAndAge
                .forEach(e -> System.out.println(e.getName() + " -> " + e.getSalary() + " -> " + e.getAge()));
    }
}
