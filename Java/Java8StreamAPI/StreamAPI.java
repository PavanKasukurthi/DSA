...
public class StreamAPI {
        public static void main(String[] args) {
                Employee e1 = new Employee(1, "Antman", 39, "Male", "DX", 2011, 100000,
                                Arrays.asList("Ruby", "Java", "C"));
                Employee e2 = new Employee(2, "Sam", 24, "Male", "ENG", 2017, 76000,
                                Arrays.asList(".Net", "JavaScript", "C++"));
                Employee e3 = new Employee(8, "Carol", 30, "Female", "CIS", 2013, 85000,
                                Arrays.asList("Java", "C#", "python"));
                Employee e4 = new Employee(3, "Tony", 25, "Male", "HR", 2010, 90000,
                                Arrays.asList("Angular", "Python", "C"));
                Employee e5 = new Employee(4, "Hercules", 42, "Male", "DX", 2015, 28000,
                                Arrays.asList("Java", ".Net", "React"));
                Employee e6 = new Employee(9, "Bruce", 27, "Male", "CIS", 2020, 68000,
                                Arrays.asList("Java", "JavaScript", "C#"));
                Employee e7 = new Employee(6, "Patsy", 28, "Female", "HR", 2022, 30000,
                                Arrays.asList("C++", "Ruby", "Java"));
                Employee e8 = new Employee(7, "Natasha", 55, "Female", "HR", 2020, 53000,
                                Arrays.asList("C#", "Python", "C++"));
                Employee e9 = new Employee(5, "Wanda", 31, "Female", "ENG", 2023, 25000,
                                Arrays.asList(".Net", "Ruby", "C#"));
                Employee e10 = new Employee(10, "Vision", 32, "Robot", "CIS", 2021, 34000,
                                Arrays.asList("Java", ".Net", "C#"));
                Employee e11 = new Employee(11, "Wasp", 33, "Female", "DX", 2001, 80070,
                                Arrays.asList("Ruby", "JavaScript", "C++"));
                Employee e12 = new Employee(12, "Thor", 34, "Male", "ENG", 2018, 56000,
                                Arrays.asList(".Net", "Java", "C++"));
                Employee e13 = new Employee(13, "Thanos", 35, "Male", "CIS", 2019, 67000,
                                Arrays.asList("JavaScript", "C#", "Java"));
                ...
                double avgMaleSalary = employees.stream().filter(e -> e.getGender().equals("Male"))
                                .mapToDouble(Employee::getSalary).average().orElse(0);
                double avgFemaleSalary = employees.stream().filter(e -> e.getGender().equals("Female"))
                                .mapToDouble(Employee::getSalary).average().orElse(0);
                double avgOtherSalary = employees.stream().filter(e -> e.getGender().equals("Robot"))
                                .mapToDouble(Employee::getSalary).average().orElse(0);

                System.out.println(
                                "Average male salary: " + avgMaleSalary + " Average female salary: " + avgFemaleSalary
                                                + " Average other salary: " + avgOtherSalary);

                System.out.println();

                // PRINT THE EMPLOYEE DETAILS WITH MAX AND MIN SALARY
                System.out.println("Employee details with minimum and maximum salary");
                // Employee maxSalaryEmployee = employees.stream().max((e,f) ->
                // Double.compare(e.getSalary(), f.getSalary())).get();
                // Using Comparator
                ...
                Employee minSalaryEmployee = employees.stream()
                                .min((e, f) -> Double.compare(e.getSalary(), f.getSalary()))
                                .get();
                ...
                List<Employee> employeesAfter2015 = employees.stream().filter(e -> e.getYearOfJoining() > 2015)
                ...
                List<Employee> employeesStartingWithT = employees.stream().filter(e -> e.getName().startsWith("T"))
                                .collect(Collectors.toList());
                ...
                List<Employee> employeesWhoKnowJava = employees.stream().filter(e -> e.getTechStack().contains("Java"))
                                .collect(Collectors.toList());
                ...
                List<Employee> employeesWhoKnowJavaAndC = employees.stream()
                                .filter(e -> e.getTechStack().contains("Java") && e.getTechStack().contains("C"))
                                .collect(Collectors.toList());
                ...
                employeeMaxSalaryInDepartment
                                .forEach((k, v) -> System.out
                                                .println(k + " --> " + v.getName() + " --> " + v.getSalary()));
                ...
                Optional<Employee> secondHighestSalariedEmployee = employees.stream()
                                .sorted(Comparator.comparingDouble(Employee::getSalary).reversed())
                                .skip(1)
                                .findFirst();

                secondHighestSalariedEmployee.ifPresent(
                                e -> System.out.println("Second highest salaried employee: " + e.getName() + " -> "
                                                + e.getSalary()));
                ...
                secondHightestInDepartment.forEach((dept, emp) -> System.out.println(
                                dept + " -> " + emp.map(
                                                e -> e.getDepartment() + " -> " + e.getName() + " -> " + e.getSalary())
                ...
                Map<String, Employee> employeeMinSalaryInDepartment = employees.stream()
                                .collect(Collectors.groupingBy(Employee::getDepartment,
                                                Collectors.collectingAndThen(
                                                                Collectors.minBy((e, f) -> Double.compare(e.getSalary(),
                                                                                f.getSalary())),
                                                                e -> e.get())));

                employeeMinSalaryInDepartment
                                .forEach((k, v) -> System.out
                                                .println(k + " --> " + v.getName() + " --> " + v.getSalary()));
                ...
                List<Employee> employeesWithSalaryGreaterThan5000 = employees.stream()
                                .filter(e -> e.getSalary() > 50000)
                                .collect(Collectors.toList());

                employeesWithSalaryGreaterThan5000
                ...
                List<Employee> employeesTechStackStartingWithR = employees.stream()
                                .filter(e -> e.getTechStack().stream().anyMatch(t -> t.startsWith("R")))
                                .collect(Collectors.toList());
                ...
                List<Employee> employeesWhoKnowJavaAndCInDX = employees.stream()
                                .filter(x -> x.getDepartment().equalsIgnoreCase("DX")
                                                && x.getTechStack().contains("Java")
                                                && x.getTechStack().contains("C"))
                                .collect(Collectors.toList());
                ...
                List<Employee> employeesNameStartingWithTInENG = employees.stream()
                                .filter(e -> e.getDepartment().equalsIgnoreCase("ENG") && e.getName().startsWith("T"))
                                .collect(Collectors.toList());
                ...
                List<Employee> employeesWhoseNameContainsU = employees.stream()
                                .filter(e -> e.getName().contains("u") || e.getName().contains("U"))
                                .collect(Collectors.toList());
                ...
                List<List<Object>> employeesWithSameTechStacks = employees.stream()
                                .flatMap(e -> employees.stream()
                                                .filter(f -> !f.equals(e)
                                                                && f.getTechStack().containsAll(e.getTechStack()))
                                                .map(f -> Arrays.asList(e.getName(), f.getName(), e.getTechStack())))
                                .distinct()
                                .collect(Collectors.toList());

                employeesWithSameTechStacks.forEach(e -> System.out
                                .println(e.get(0) + " and " + e.get(1) + " have the same techStacks: " + e.get(2)
                                                + "\n"));
                ...
                employeesSortedBasedOnTheirSalaries
                                .forEach(e -> System.out.println(e.getName() + " -> " + e.getSalary()));
                ...
                employeesSortedBasedOnTheirSalariesDesc
                                .forEach(e -> System.out.println(e.getName() + " -> " + e.getSalary()));

                System.out.println();

                // SORT EMPLOYEES BASED ON THEIR SALARIES AND IF THE SALARIES ARE SAME THEN SORT
                // THEM BASED ON THEIR AGE
                System.out.println(
                                "Sort the employees based on their salaries and if the salaries are same then sort them based on their age ");
                List<Employee> employeesSortedBasedOnTheirSalariesAndAge = employees.stream()
                                .sorted((e, f) -> e.getSalary() == f.getSalary()
                                                ? Integer.compare(e.getAge(), f.getAge())
                                                : Double.compare(e.getSalary(), f.getSalary()))
                                .collect(Collectors.toList());
